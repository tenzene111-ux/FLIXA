...
// denormalized counter — gift volume per video is small enough at MVP
// scale that this stays cheap and can never drift out of sync.
export function subscribeToGiftLeaderboard(videoId: string, onChange: (entries: GiftLeaderboardEntry[]) => void) {
  const giftsQuery = query(collection(db, 'videos', videoId, 'gifts'), orderBy('createdAt', 'desc'));
